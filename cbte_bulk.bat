...
echo [!COUNT!/%TOTAL%] !PERCENT!%% Extracting: %~1
"%CBTE%" -e "%~1"
if errorlevel 1 (
    echo   FAILED: %~1
    set /a FAIL+=1
...
set "CFG=!JSON:.json=!"
if exist "!CFG!" (
    echo [!COUNT!/%TOTAL%] !PERCENT!%% Updating: !CFG!
    "%CBTE%" -w "!CFG!" "%~1"
    if errorlevel 1 (
        echo   FAILED: !CFG!
        set /a FAIL+=1
    ...
    echo [!COUNT!/%TOTAL%] !PERCENT!%% Skipped: %~1
    set /a FAIL+=1
)
goto :eof
...
:usage
echo Usage:
echo   cbte_bulk.bat -e ^<folder^>    Extract all cfg.bin to json
echo   cbte_bulk.bat -w ^<folder^>    Update all cfg.bin from json
exit /b 1
